...
function App() {
  ...
  const scrollRail = (ref, direction) => {
    ref.current?.scrollBy({ left: direction * Math.min(ref.current.clientWidth * 0.82, 420), behavior: 'smooth' });
  };
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#070707] text-white">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-black/72 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8">
          ...
          <div className="flex items-center gap-2">
            <a href="#contact" className="hidden rounded-full border border-white/20 px-4 py-2 text-xs uppercase tracking-[0.16em] transition hover:bg-white hover:text-black sm:block">Contact</a>
            <button aria-label="Toggle menu" aria-expanded={menuOpen} onClick={() => setMenuOpen((v) => !v)} className="grid h-10 w-10 place-items-center rounded-full border border-white/15 lg:hidden">
              <span className="sr-only">Menu</span>
              <span className={`mobile-menu-lines ${menuOpen ? 'open' : ''}`}><i/><i/></span>
            </button>
          </div>
        </div>
        <div className={`mobile-nav lg:hidden ${menuOpen ? 'open' : ''}`}>
          <nav className="mx-auto grid max-w-7xl gap-1 px-5 pb-5 pt-1 md:px-8">
            ...
            <a onClick={() => setMenuOpen(false)} href="#contact" className="mt-3 rounded-full border border-amber-300/40 px-4 py-3 text-center text-xs uppercase tracking-[.2em] text-amber-300">Contact / Bookings</a>
          </nav>
        </div>
      </header>
      ...
      <main id="top">
        <section className="hero-stage relative min-h-screen overflow-hidden px-5 pb-12 pt-28 md:px-8 md:pb-20">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(3,3,3,.99)_0%,rgba(3,3,3,.82)_42%,rgba(3,3,3,.18)_72%,rgba(3,3,3,.68)_100%)]" />
          ...
          <div className="absolute inset-y-0 right-4 hidden items-center xl:flex"><div className="rotate-90 text-[10px] uppercase tracking-[.5em] text-white/30">Fashion · Music · Culture · Kingston to the world</div></div>

          <div className="relative z-10 mx-auto flex min-h-[calc(100vh-7rem)] max-w-7xl flex-col justify-end">
            <div className="mb-8 flex items-center gap-4 text-[10px] uppercase tracking-[.35em] text-white/40"><span>Est. 2022</span><span className="h-px w-14 bg-white/20"/><span>Dancehall lifestyle label</span></div>
            <div className="grid items-end gap-10 lg:grid-cols-[1.25fr_.75fr]">
              <div>
                ...
                <h1 className="hero-title max-w-5xl text-[19vw] font-black uppercase leading-[.73] tracking-[-.07em] sm:text-[14vw] lg:text-[10.3rem]"><span>Diggy</span><br/><span className="text-stroke">Nation</span></h1>
                <div className="mt-8 flex max-w-3xl flex-col gap-7 border-l border-white/15 pl-5 md:flex-row md:items-end md:justify-between md:pl-7">
                  <p className="max-w-xl text-base leading-7 text-white/72 md:text-lg">Dancehall heritage, fashion, music and culture — built into one digital home.</p>
                  <div className="text-right text-[10px] uppercase tracking-[.26em] text-white/35">Original media archive<br/>restored 2026</div>
                </div>
              ...
              <div className="pb-2 lg:pb-8">
                <Reveal delay={220}>
                  <div className="editorial-card border-l border-amber-300/70 bg-black/28 p-5 backdrop-blur-md">
            ...
            <a href="#shop" className="mt-10 inline-flex w-fit items-center gap-3 text-[10px] uppercase tracking-[.28em] text-white/35 transition hover:text-white/70"><span className="scroll-dot"/>Scroll to explore</a>
          </div>
        </section>
        ...
        <section id="shop" className="mx-auto max-w-7xl px-5 py-24 md:px-8">
          ...
          <div ref={productRail} className="horizontal-rail snap-x snap-mandatory">
            {products.map((product, i) => (
              <div key={product.name} className="w-[82vw] max-w-[350px] shrink-0 snap-start sm:w-[340px]">
                <SpotlightCard className="group !min-h-[500px] !overflow-hidden !border-white/10 !bg-[#0d0d0d] !p-0" spotlightColor="rgba(245,181,45,.20)">
                  <div className="relative z-10 flex min-h-[498px] flex-col">
                    <div className="relative h-72 overflow-hidden bg-[#111]">
                      <img src={media(productImages[product.name])} alt={product.name} loading="lazy" className="image-reveal h-full w-full object-cover transition duration-700 group-hover:scale-105" />
        ...
        <section id="about" className="border-y border-white/10 bg-[#0b0b0b]">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-24 md:px-8 lg:grid-cols-[.9fr_1.1fr]">
            <Reveal className="grid grid-cols-2 gap-3">
              {galleryImages.slice(0, 4).map((img, i) => <div key={img} className={`archive-frame overflow-hidden rounded-2xl ${i === 0 ? 'col-span-2 aspect-[16/9]' : 'aspect-[4/5]'}`}><img src={media(img)} alt={`Diggy Nation / Mr. Lexx archive ${i + 1}`} loading="lazy" className="h-full w-full object-cover" /></div>)}
            </Reveal>
            <Reveal delay={100}>
              ...
              <div className="mt-10 space-y-5">{about.map((item) => <div key={item.title} className="border-l border-white/15 pl-5"><h3 className="text-xl font-bold">{item.title}</h3><p className="mt-2 leading-7 text-white/50">{item.body}</p></div>)}</div>
            </Reveal>
          </div>
        </section>
        ...
        <section id="mr-lexx" className="mx-auto max-w-7xl px-5 py-24 md:px-8">
          <div className="grid gap-12 lg:grid-cols-2">
            <Reveal className="relative min-h-[620px] overflow-hidden rounded-[2rem] border border-white/10">
              <img src={media(lexxPortrait)} alt="Mr. Lexx portrait" loading="lazy" className="image-reveal absolute inset-0 h-full w-full object-cover object-top" />
          ...
          <div className="mt-10 flex items-center justify-between"><p className="text-[10px] uppercase tracking-[.24em] text-white/30">Archive gallery</p><div className="flex gap-2"><button onClick={() => scrollRail(galleryRail, -1)} className="rail-btn">←</button><button onClick={() => scrollRail(galleryRail, 1)} className="rail-btn">→</button></div></div>
          <div ref={galleryRail} className="horizontal-rail mt-4 snap-x snap-mandatory">{galleryImages.map((img, i) => <div key={img} className="w-[62vw] max-w-[300px] shrink-0 snap-start overflow-hidden rounded-2xl border border-white/10 sm:w-[260px]"><img src={media(img)} alt={`Mr. Lexx archive ${i + 1}`} loading="lazy" className="aspect-[4/5] w-full object-cover transition duration-700 hover:scale-105" /></div>)}</div>
        </section>

        <section id="press" className="border-y border-white/10 bg-[#0b0b0b]">
          <div className="mx-auto max-w-7xl px-5 py-24 md:px-8">
            <Reveal><p className="text-xs uppercase tracking-[.35em] text-amber-300">Press archive</p><div className="mt-4 grid gap-10 lg:grid-cols-[.65fr_1.35fr]"><div><h2 className="text-5xl font-black uppercase leading-[.9] tracking-[-.05em] md:text-7xl">Mr. Lexx<br/>in the news.</h2><p className="mt-6 max-w-md text-sm leading-7 text-white/45">Recovered imagery from the original Diggy Nation press archive now lives inside this rebuild.</p></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{press.slice(0, 6).map((item, i) => <article key={`${item.date}-${item.title}`} className="group overflow-hidden rounded-2xl border border-white/10 bg-[#111] transition duration-500 hover:-translate-y-1 hover:border-white/20"><div className="aspect-[4/3] overflow-hidden"><img src={media(pressImages[i])} alt={item.title} loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /></div><div className="p-5"><div className="text-[10px] uppercase tracking-[.18em] text-amber-300/70">{item.date || 'Archive'}</div><h3 className="mt-2 text-sm font-semibold leading-6 text-white/78">{item.title}</h3></div></article>)}</div></div></Reveal>
            <div className="mt-10 divide-y divide-white/10 border-y border-white/10">{press.slice(6).map((item, i) => <Reveal key={`${item.date}-${item.title}`} delay={Math.min(i * 35, 210)}><div className="grid gap-2 py-4 md:grid-cols-[150px_1fr]"><span className="text-xs uppercase tracking-[.14em] text-white/30">{item.date || 'Archive'}</span><span className="font-medium text-white/65">{item.title}</span></div></Reveal>)}</div>
          </div>
        </section>

        <section id="events" className="mx-auto max-w-7xl px-5 py-24 md:px-8">
          <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr]">
            <Reveal><p className="text-xs uppercase tracking-[.35em] text-amber-300">Historical event archive</p><h2 className="mt-4 text-5xl font-black uppercase tracking-[-.05em] md:text-7xl">On the road.</h2><div className="mt-8 overflow-hidden rounded-[2rem]"><img src={media('media/mr-lexx/mr-lexx-gallery-19.jpg')} alt="Mr. Lexx archive" loading="lazy" className="aspect-[4/5] w-full max-w-md object-cover transition duration-700 hover:scale-105" /></div></Reveal>
            <div className="grid gap-3 md:grid-cols-2">{events.map((event, i) => <Reveal key={event} delay={Math.min(i * 25, 180)}><div className="rounded-2xl border border-white/10 bg-white/[.025] p-5 text-sm leading-6 text-white/55 transition hover:border-amber-300/30 hover:bg-amber-300/[.03]">{event}</div></Reveal>)}</div>
          </div>
        </section>

        <section id="reviews" className="border-y border-white/10 bg-[#0b0b0b]">
          <div className="mx-auto max-w-7xl px-5 py-24 md:px-8">
            <Reveal className="flex flex-wrap items-end justify-between gap-6"><div><p className="text-xs uppercase tracking-[.35em] text-amber-300">Community feedback</p><h2 className="mt-4 text-5xl font-black uppercase tracking-[-.05em] md:text-7xl">77 reviews.</h2></div><p className="max-w-lg text-sm leading-6 text-white/45">Selected feedback from the original Diggy Nation site.</p></Reveal>
            <div className="mt-10 grid gap-5 md:grid-cols-3">{reviews.slice(0, 9).map(([name, date, quote], i) => <Reveal key={`${name}-${date}`} delay={Math.min(i * 45, 180)}><SpotlightCard className="!border-white/10 !bg-[#111] !p-6" spotlightColor="rgba(245,181,45,.14)"><div className="relative z-10"><p className="leading-7 text-white/65">“{quote}”</p><div className="mt-6 text-sm font-semibold">{name}</div><div className="mt-1 text-xs text-white/30">{date}</div></div></SpotlightCard></Reveal>)}</div>
          </div>
        </section>

        <section className="relative overflow-hidden px-5 py-24 md:px-8">
          <img src={media('media/mr-lexx/mr-lexx-gallery-01.jpg')} alt="Mr. Lexx" loading="lazy" className="parallax-lite absolute inset-0 h-full w-full object-cover opacity-20" />
          <div className="absolute inset-0 bg-[linear-gradient(90deg,#080604_20%,rgba(8,6,4,.82)_55%,rgba(8,6,4,.72))]" />
          <Reveal className="relative mx-auto max-w-7xl rounded-[2.5rem] border border-amber-300/25 bg-black/45 p-8 backdrop-blur-sm md:p-14">
